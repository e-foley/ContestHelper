...
public class MemberSortConsecutiveLoose implements MemberDataRetriever
{
    public float getValue(Member member) {
        return member.getLongestWinStreak(false);
    }
    
    public int compare(Member m1, Member m2) {
        int result = new Float(m2.getLongestWinStreak(false)).compareTo(m1.getLongestWinStreak(false));
        if (result == 0) {
            result = m2.getNumberOfLongestWinStreaks(false) - m1.getNumberOfLongestWinStreaks(false);
        }
        return result;
    }
    ...
    {
        if (m.getNumberOfLongestWinStreaks(false) > 1) {
            return "(" + NumberFormat.getInstance().format(m.getNumberOfLongestWinStreaks(false)) + "&times;)&nbsp;" + getFormat().format(m.getLongestWinStreak(false));
        } else {
            return getFormat().format(m.getLongestWinStreak(false));
        }
    }
    ...
    public String getDetails(Member m, boolean linkTopics)
    {
        ...
        ArrayList<ArrayList<Member.EntryStakePair>> winners = m.getEntriesInLongestWinStreak(false);
